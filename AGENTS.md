...
- **The PoC is complete.** Its roadmap (phases T1–T16) and its acceptance criteria (CA-1…CA-17, in `internal-docs/alcance-poc-vertice.md`) are now a historical record of a finished stage, not a plan to trace new work against — the roadmap document itself is no longer in the repository, and archived changes under `openspec/changes/archive/` still cite it as history. Work from here is the **final product**: scope it from the current living specs in `openspec/specs/` and from the change proposal at hand. Where a spec still says "Traces to T*n* of the completed PoC roadmap", read that as provenance, not as a constraint on new work. The PoC's *out-of-scope* list in `internal-docs/alcance-poc-vertice.md` no longer governs; the structural invariants below still do.
...
- The openspec folder contains the decision history, specifications already implemented, and specifications currently being implemented.
- Use **compendio-mcp** to consult the project's OpenSpec documentation, and the others folder docs like internal-docs or docs