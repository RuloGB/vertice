# OpenCode Agent Scanner Specification (Delta)

## Purpose

Defines the contract for discovering OpenCode agent `Component` values from the `agent` object embedded in two configuration files, `opencode.json` and `opencode.jsonc`, merged per key. Traces to T6 of `internal-docs/plan-desarrollo-poc.md`; closes the OpenCode half of CA-5 (an agent defined only in `opencode.jsonc` appears alongside `opencode.json`-only agents); contributes to CA-12 (a malformed file yields a `ScanIssue` carrying its path and interrupts nothing); bound by CA-16 (read-only) and CA-17 (fixture-based, machine-independent tests, no reuse of another scanner's fixture tree). Core (Rust) only — no IPC or frontend surface in this change; no `domain-model` requirement is added, modified, or exercised differently by this capability.
