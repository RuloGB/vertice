...
- [x] `cargo test` passes on all three CI platforms with no dependency, `deny.toml`, or CI-workflow change. **Verified**: PR #5 (`feat(core): SKILL.md frontmatter reader (T3, 2 of 2 — re-targeted at main)`) passed all six CI jobs (`frontend`, `quality`, `msrv (1.88)`, and `rust` jobs on `macos-14`, `ubuntu-24.04`, `windows-2022`). Test count confirmed as 60 tests green. PR #3 (`feat(frontmatter-fixtures)`) passed the same six CI jobs on the three-platform matrix. `Cargo.toml`, `deny.toml`, and `.github/workflows/ci.yml` untouched — zero new dependencies, zero CI workflow changes.
