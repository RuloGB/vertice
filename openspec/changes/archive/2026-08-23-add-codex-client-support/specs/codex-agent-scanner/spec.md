# Delta for Codex Agent Scanner

New capability. Traces to T7's third-client extension of the T5/T6 per-client
agent-adapter pattern (`internal-docs/plan-desarrollo-poc.md:132-167`); closes
CA-12 (a corrupt file is reported with its path and does not break the scan)
for the new TOML dialect, and CA-8 (no name-convention filtering) re-affirmed
for a third vendor. Bound by CA-16 (read-only) and CA-17 (versioned fixtures).
Core (Rust) only — no IPC or frontend surface; `ClientKind::Codex` and its
regenerated binding are specified by the `domain-model` delta, not here.
...
parsed through the new `toml.rs` seam (specified by the `workspace-architecture`
delta) rather than through `frontmatter::read`. It is a third standalone
adapter module, not an extension of `agents.rs` or `opencode_agents.rs`, and
introduces no shared abstraction across the three agent adapters.

The exact mapping from TOML keys to `Component.name` / `description` /
`provenance_hint`, and the behavior for a file missing its name key, are
`sdd-design` decisions and are deliberately not pinned by this spec beyond
the observable guarantees below.
...
already established. Whether this root is represented as a `SearchRoot` and,
if so, which `SearchRootKind` it carries is a design-phase decision; this
requirement binds only the resolved path.
...
quote or the first newline. The mapping from TOML keys onto the resulting
DTO's fields, and the DTO's required/optional field set, are defined in
`design.md`.
...
recorded in the parent proposal).
...
Each discovered `.toml` file that fails to parse, or that fails whatever
required-field validation `design.md`'s DTO defines, MUST produce exactly one
`ScanIssue` at `IssueSeverity::Error` carrying that file's path, mirroring
`agent-scanner`'s and `skill-scanner`'s escalation rule, and MUST NOT prevent
any sibling `.toml` file under the same root from being discovered and
parsed (CA-12).
