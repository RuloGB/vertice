...
All ten core types (`Component`, `ComponentKind`, `Scope`, `Location`, `SearchRoot`, `ClientInstallation`, `ClientPresence`, `ClientPresenceStatus`, `ScanIssue`, `ScanReport`) and their nested enums, including `ClientKind`, MUST derive `Serialize`, `Deserialize`, and `TS`. Generated TypeScript bindings MUST be checked into `frontend/src/bindings/` and MUST structurally mirror the Rust definitions: field names, optionality, and closed union variants. `frontend/src/bindings/ClientKind.ts` MUST be regenerated by `cargo test -p vertice-core` in the same commit as any change to the Rust enum, and MUST NEVER be hand-edited; CI's drift gate MUST fail if it is not regenerated.
